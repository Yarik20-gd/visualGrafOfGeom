...
let surface, surfaceL, lineStr;                    // A surface model
...
function draw() {
    ...
    gl.uniform3fv(shProgram.iP, [Math.cos(Date.now() * 0.001), document.getElementById('pY').value, -1]);
    ...
    surface.Draw();
    gl.uniform1f(shProgram.iS, 20);
    gl.uniformMatrix4fv(shProgram.iModelViewProjectionMatrix, false, m4.multiply(modelViewProjection,m4.translation(Math.cos(Date.now() * 0.001), document.getElementById('pY').value, -1)));
    surfaceL.Draw();
    gl.uniformMatrix4fv(shProgram.iModelViewProjectionMatrix, false, modelViewProjection);
    lineStr.BufferData([Math.cos(Date.now() * 0.001), document.getElementById('pY').value, -1, Math.cos(Date.now() * 0.001), document.getElementById('pY').value, 0])
    gl.uniform1f(shProgram.iS, 40);
    lineStr.DrawStr()
}
function drawAgain() {
    draw()
    window.requestAnimationFrame(drawAgain)
}
let infinity = 1;
let numOfSteps = 10;
...
function initGL() {
    ...
    surfaceL = new Model('Surface');
    surface.BufferData(...CreateSurfaceData());
    centerOfMass[0] /= surface.count
    centerOfMass[1] /= surface.count
    centerOfMass[2] /= surface.count
    surfaceL.BufferData(CreateSphereData(), CreateSphereData());
    lineStr = new Model()
    lineStr.BufferData([0, 0, 0, 1, 1, 1])
